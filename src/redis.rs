...
impl Redis {
    pub fn new(cfg: Config) -> Result<Self, String> {
        let client = redis::Client::open(cfg.generate_url()).expect("Failed to make connection with redis server");
        Ok(Self {
            client,
        })
    }
    ...
    pub fn get(&self, key: &str) -> Result<String, String> {
        ...
        connection.get(key).map_err(|e| {
            format!("Failed to get {}. Error => {}", key, e)
        })
    }
}
